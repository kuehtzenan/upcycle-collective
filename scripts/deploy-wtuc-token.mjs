...
// Load organiser keypair from env (never hardcode in source). Mint authority
// AND freeze authority are both set to this single keypair's pubkey below —
// not a multisig, not a PDA — so only whoever holds ORGANISER_PRIVATE_KEY can
// ever mint or freeze wTUC.
...
async function main() {
  ...
  const mint       = Keypair.generate(); // new random mint account — independent of the existing TUC mint
  ...
  const tx = new Transaction().add(
    ...
    // 3. Initialize the mint — organiser is BOTH mint authority and freeze
    // authority, as a single-signer pubkey (not a multisig account), so
    // minting strictly requires ORGANISER_PRIVATE_KEY and nothing else.
    createInitializeMintInstruction(
      mint.publicKey,
      TOKEN_DECIMALS,
      organiser.publicKey,
  ...
  console.log('✅  wTUC deployed!');
  console.log('Mint address:', mint.publicKey.toBase58());
  console.log('Explorer:    ', `https://explorer.solana.com/address/${mint.publicKey.toBase58()}?cluster=${NETWORK}`);
  console.log('Signature:   ', signature);
  console.log('\n⬇️  Copy this mint address into TOKEN_MINT (server env) and the hardcoded');
  console.log('   MINT constants in public/*.html (app.html, staff.html, vendor.html,');
  console.log('   dashboard.html, hunt.html) once the codebase changes are in place.');
}
